    ...
    def order_material(self, provider, inventory, daily_events):
        ...
        while True:
            ...
            if order_size > 0:
                    ...
                    daily_events.append(f"{self.env.now}: RAW MATERIAL 1\'s Intransition_inventory                  : {inventory.intransition_inventory} units ")#Record intransition_inventory
            
                    daily_events.append(f"{self.env.now}: RAW MATERIAL 1\'s Real_Inventory                          : {inventory.inventory} units  ") #Record inventory
            daily_events.append(
...
class Sales:
    ...
    def deliver_to_cust(self, demand_size, product_inventory, daily_events):
        ...
        if product_inventory.on_hand_inventory < demand_size:
            num_shortages = abs(product_inventory.on_hand_inventory - demand_size)
